function [AdInfo,AdInfo_names] = Spikelet_AD_TrTe_STOMP_ver_04(SegInfo,training_region,Param)                                 
  FuncName = 'Spikelet_AD_TrTe_STOM_ver_04';
  AdInfo_names = ["id","from","to","type","magnitude","dist","distLengthNorm","from_NN","to_NN","spike_sign","testNum_possible","testNum_restricted","testNumRate","adjust","distLengthNorm_pre"];
  ...
  if isfield(Param,"baseline")
   EXCLUSION_WINDOW_RATE = Param.baseline.EXCLUSION_WINDOW_RATE;  
  else
    EXCLUSION_WINDOW_RATE = 0.2;
  end
  if Param.ablation.znormBias
    % PV0 Flac
    EXCLUSION_FlacToFlac_0_RATE = Param.znormBias.FlacToFlac_0;
    
    % PV1 Flac
    EXCLUSION_FlacToFlac_1_RATE = Param.znormBias.FlacToFlac_1;

    % PV2 Flac
    EXCLUSION_FlacToFlac_2_RATE = Param.znormBias.FlacToFlac_2;
    
    % PV3 Flac
    EXCLUSION_FlacToFlac_3_RATE = Param.znormBias.FlacToFlac_3;

    % PV5 Flac
    EXCLUSION_FlacToFlac_5_RATE = Param.znormBias.FlacToFlac_5;

    % PV2 Mat
    EXCLUSION_MatToMat_RATE = Param.znormBias.MatToMat_2;
  else
    EXCLUSION_FlacToFlac_0_RATE = [-inf,inf];
    EXCLUSION_FlacToFlac_1_RATE = [-inf,inf];
    EXCLUSION_FlacToFlac_2_RATE = [-inf,inf];
    EXCLUSION_FlacToFlac_3_RATE = [-inf,inf];
    EXCLUSION_FlacToFlac_5_RATE = [-inf,inf];
    EXCLUSION_MatToMat_RATE =  [-inf,inf];  
  end
  ...
  for i = 1:size(SegDB,1)  % i:test, j:train
    ...
    %  AdInfo_names = ["id","from","to","type","magnitude","dist","distLengthNorm","from_NN","to_NN","spike_sign","testNum_possible","testNum_restricted","testNumRate","adjust","distLengthNorm_pre"];
    testNum_possible_i = length(DistanceProfile_i);
    testNum_restricted_i = length(DistanceProfile_i) - sum(EXCLUSION_ZONE_i);
    testNumRate_i = testNum_restricted_i/testNum_possible_i;
    adInfo_i = [spikeId_i,from_i,to_i,type_i,mag_i,min_dist_i,min_dist_lengthNorm_i,fromNN_i,toNN_i,sign_i,testNum_possible_i,testNum_restricted_i,testNumRate_i];
